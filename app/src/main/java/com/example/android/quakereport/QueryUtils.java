...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link EarthquackData} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthquackData> extractEarthquakes() {
        ...
        try {
            ...
            for(int i=0; i<earthquackArray.length();i++){
                ...
                JSONObject propertics = currentEarthquack.getJSONObject("properties");
                Double magnitude = propertics.getDouble("mag");
                String location = propertics.getString("place");
                Long time = propertics.getLong("time");
                String url = propertics.getString("url");
                EarthquackData earthquack = new EarthquackData(magnitude, location, time, url);
